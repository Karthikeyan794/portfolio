...
- **Your take** has the meadow with the paper plane behind it, sharp (the demo's own wallpaper,
  `public/work/support-desk/demo-bg.jpg`, path in `feedback.bg`), and one **300px glass card** in its
  middle holding the form: "Share your words about it", the note, and in one row two small glass
  thumb buttons (the same 30px circle as the expand button on the recordings; chosen = white glass)
  and a white Send. All black and white, no accent colour; the note's focus line is off-white. The
  inbox fine print is gone.
- Every tile's label (Up next, Keep going, Your take, Contact) sits top left, in the same place.
- **12+ more projects** has the Milky Way behind it (darkened in the middle for the number) in place
  of the drawn light beam; **Contact** has the night library the contact dialog sits on
  (`/contact/night-library.webp`), so the tile and the dialog it opens are one place. Both paths are
  in `endTiles` in `src/data.ts`.
...
- [x] ~~**Snowfall** over the landing screen~~ → fireflies since 25 Sep; removed from About too on 26 Sep (the snow code and `snowflake.svg` are gone). Was: — smooth canvas flakes (drawn icy crystal sprite at `public/snowflake.svg`; drop your own PNG there to swap), depth-based speed, sway, spin; sleeps off-screen and in hidden tabs; off for reduced-motion users
