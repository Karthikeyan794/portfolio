...
**Copy answers the cursor.** Every word of case-study copy is its own `<span class="hw">` — a soft band of the accent (13%) washes in behind the word under the pointer, left to right, and the word comes up to full contrast. Nothing moves, so a sentence stays perfectly still while you read across it. Pure CSS (one background gradient), no JS, and no transition under `prefers-reduced-motion`. (First version lifted the word 2px with a lime underline; too busy over a whole page of copy.) Wrapping lives in `src/components/Words.tsx`; it also handles `*marked*` runs, so the marked phrase keeps its own colour when you touch it. Applied to the overview paragraph, the six feature rows, problems/solutions, every story row and the closing block. To add it anywhere else: `<Words text={...} />` instead of `{...}`.
