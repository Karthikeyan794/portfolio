...
- **The pointer is a drawn one.** Over live copy the system arrow is swapped for a hand-drawn pointer — cream body, brown outline, white sticker edge, three amber sparks off the tip — matching the picture you sent. It is a real PNG (`public/cursor/pointer.png`, with a 2x file handed over by `image-set()`), not an SVG cursor, which only Chrome handles dependably. The hotspot is `10 9`: the drawn tip, not the corner of the image. Redrawn from the SVG in the scratch file if it ever needs changing; keep the tip where it is or update the hotspot with it. (Before this it was a sparkle emoji, and before that the Figma and Claude marks split across the page.)
