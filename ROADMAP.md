...
- **The pointer names the tool.** Over live copy the arrow becomes the Figma mark across the design half of the page (overview, features, problems and solutions) and the Claude mark across the build half (the flow-by-flow story and the closer). Real PNGs in `public/cursor/`, each with a 2x file handed over by `image-set()` — not SVG cursors, which are only dependable in Chrome. 28px, hotspot at the centre, text caret as the last fallback.
  - The Figma mark is its standard five-shape geometry. **The Claude mark is my own drawing** — eleven tapered rays in Claude's coral, close enough at 28px but not the official file. Drop the real one in at the same path and size if you want it exact. Both are trademarks of their owners; they are here as "made with", which is what that use is for.
